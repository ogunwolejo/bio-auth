...
describe("AuthResolver", () => {
  ...
  const mock: UsersModel = {
    id: 1,
    email: "jayjay@gmail.com",
    password: "hashedpassword", // This would be a hashed password in production
    name: "jay jay",
  ...
  describe("login", () => {
    it("should return a user model when login is successful", async () => {
      ...
      authService.validateUser = jest.fn().mockResolvedValue(mock);
    ...
    it("should throw an error if login fails", async () => {
      ...
      await expect(authResolver.login(input)).rejects.toThrow(
        "Invalid credentials",
      );
    });
  });
  ...
  describe("registerUser ", () => {
    ...
    it("should throw an error if registration fails", async () => {
      ...
      await expect(authResolver.registerUser(input)).rejects.toThrow(
        GraphQLError,
      );
      await expect(authResolver.registerUser(input)).rejects.toThrow(
        "Registration failed",
